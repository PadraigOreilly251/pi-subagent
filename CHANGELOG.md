...
| Model pinning | `runner.ts` | Child receives `--provider/--model/--thinking` derived from the parent session, because pi does *not* read `PI_PROVIDER`/`PI_MODEL`/`PI_REASONING_LEVEL` — previously every child silently ran on the `settings.json` default (observed: parent on one inference host, children on another). `PI_SUBAGENT_MODEL` / `PI_SUBAGENT_PROVIDER` / `PI_SUBAGENT_THINKING` override. |
...
| `/submodel` command | `index.ts` | Interactive menu to pin the model sub-agent children run on (no arg = picker, `reset`/`show` args). Default: inherit the parent's live model. Pin is per-session — `session_start` resets it to inherit. When pinned, the tool result is tagged `⚙ subagent model: <model> (pinned via /submodel)`. |
| `/subconcurrency` command | `index.ts` | Live concurrency gate (`/subconcurrency N`, alias `/submodelnumber`). `0` = serial/blocking (main workflow on hold until the child finishes), `N` = up to N concurrent. Default seeded from `PI_SUBAGENT_MAX_PARALLEL`; the command overrides it for the process lifetime (does not reset on new session). |
